...
def run():
    ...
    create_blob_response = resources_server.client_caller(
        upload_stub.CreateBlob, blob_spec, "CreateBlob")
    ...
    # GetBlobInfo
    info_response = resources_server.client_caller(download_stub.GetBlobInfo,
                                                   blob_id, "GetBlobInfo")
    ...
    chunks = device.create_chunks(image_filename, chunk_count, chunk_size,
                                  blob_id)
    ...
    for i in range(0, chunk_count):
        chunk = chunks[i]
        upload_response = resources_server.client_caller(
            upload_stub.UploadChunk, chunk, "UploadChunk")
        # Check there were no issues
    ...
    for i in range(0, chunk_count):
        ...
        download_response = resources_server.client_caller(
            download_stub.GetChunk, chunk_spec, "GetChunk")
    ...
    command_response = resources_server.client_caller(
        upload_stub.GetAverageBrightness, blob_id, "GetAverageBrightness")
    avg_brightness = device.bytes_to_int(command_response.payload)
    print("    Result: %i" % avg_brightness)

    # Delete the blob
    print("\nDeleting the blob")
    delete_response = resources_server.client_caller(upload_stub.DeleteBlob,
                                                     blob_id, "DeleteBlob")
    ...
    download_response = resources_server.client_caller(download_stub.GetChunk,
                                                       chunk_spec, "GetChunk")
    ...
    measurement_response = resources_server.client_caller(
        download_stub.GetMeasurementData, binary_data_pb2.Empty(),
        "GetMeasurementData")
